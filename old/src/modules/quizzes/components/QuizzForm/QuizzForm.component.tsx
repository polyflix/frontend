...
export const QuizzForm = ({ quizz, isUpdate }: Props) => {
  ...
  const onSubmit = async (data: IQuizzForm) => {
    ...
    try {
      // Validate questions
      if (_quizz.questions && !_quizz.questions.every(Question.validate)) {
        setError(t("quizzes.form.errors.questions.noCorrectAlternatives"));
      } else {
        if (isUpdate) await quizzService.updateQuizz(quizz?.id!, _quizz);
        else await quizzService.createQuizz(_quizz);
