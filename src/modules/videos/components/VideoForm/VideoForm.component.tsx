...
export const VideoForm: React.FC<Props> = ({ video }) => {
  ...
  const save = async () => {
    ...
    const videoElement = provider?.lastChild?.lastChild as HTMLVideoElement

    let canvas = document.createElement('canvas')
    let context = canvas.getContext('2d')
    ;[canvas.height, canvas.width] = [720, 1280]

    context?.drawImage(videoElement as CanvasImageSource, 0, 0, 1280, 720)
    canvas.toBlob((blob) => {
      if (blob) {
        const file = new File(
          [blob],
          `capture_at_${videoElement.currentTime}sec`
        )
